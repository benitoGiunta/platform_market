...
import { materielsService as service } from "../services/materiels.service.js";
import { ok, fail } from "../lib/reply.js";

export const materielsController = {
  ...
  async findById(req: Request, res: Response) {
    ...
    if (!item) return fail(res, "NOT_FOUND", "Matériel introuvable", 404);
    return ok(res, item);
  },
  ...
  async linkVideaste(req: Request, res: Response) {
    const link = await service.linkVideaste(Number(req.params.id), Number(req.body.videaste_id));
